...
\definecolor{bigpurple}{RGB}{160, 120, 160} % 稍微浓一点的紫色
...
\definecolor{bar1}{RGB}{41,128,185}
\definecolor{bar2}{RGB}{46,204,113}
\definecolor{bar3}{RGB}{155,89,182}
\definecolor{bar4}{RGB}{243,156,18}
\definecolor{bar5}{RGB}{231,76,60}
...
{\normalsize{\color{customTeal}\faInfoCircle} 肠道菌群可以大分类成细菌，病毒，真菌，原生生物和寄生虫5个类别，下列是对各项组成部分的介绍及他们在您肠道中的分布比例图。
\begin{itemize}
    \item \textcolor{bar1}{\textbf{细菌}}：细菌是肠道微生物的主要组成部分，
    \item \textcolor{bar2}{\textbf{病毒}}：病毒，尤其是噬菌体，在调节细菌的种类和数量方面扮演重要角色。它们既可以抑制有害细菌的生长，也可以推动微生物群落的多样性。
    \item \textcolor{bar3}{\textbf{真菌}}：主要包括酵母菌（如白色念珠菌），在数量较少时是正常的一部分，但当免疫系统受损时，可能会因其快速繁殖而导致感染或其它问题。
    \item \textcolor{bar4}{\textbf{原生生物}}：数量较少，但原生生物如变形虫和细胞虫有时会影响肠道环境，参与营养循环和分解。
    \item \textcolor{bar5}{\textbf{寄生虫}}：常见的肠道寄生虫如蛔虫和鞭虫，虽然数量较少，但在某些情况下可能导致消化不良和腹痛等健康问题。
\end{itemize}
}
\end{tcolorbox}
...
{\normalsize 下列是对各项组成部分的介绍及他们在您肠道中的分布比例图。
}
\end{tcolorbox}
\vspace{0.5cm}
...
%    {
%        \draw[gray!20] (0,\y*3) -- (\textwidth,\y*3);
%    }

    % 绘制圆角条形图（增加高度）
    \fill[bar1,rounded corners=4pt] (0,15) rectangle (0.9*\textwidth,16.8);
    \fill[bar2,rounded corners=4pt] (0,12) rectangle (0.05*\textwidth,13.8);
    \fill[bar3,rounded corners=4pt] (0,9) rectangle (0.03*\textwidth,10.8);
    \fill[bar4,rounded corners=4pt] (0,6) rectangle (0.01*\textwidth,7.8);
    \fill[bar5,rounded corners=4pt] (0,3) rectangle (0.01*\textwidth,4.8);

    % 添加标签（带背景框）
    \node[anchor=east,fill=white,inner sep=2pt,text=bar1] at (-0.2,15.9) {\Large\textbf{细菌}};
    \node[anchor=east,fill=white,inner sep=2pt,text=bar2] at (-0.2,12.9) {\Large\textbf{病毒}};
    \node[anchor=east,fill=white,inner sep=2pt,text=bar3] at (-0.2,9.9) {\Large\textbf{真菌}};
    \node[anchor=east,fill=white,inner sep=2pt,text=bar4] at (-0.2,6.9) {\Large\textbf{原生生物}};
    \node[anchor=east,fill=white,inner sep=2pt,text=bar5] at (-0.2,3.9) {\Large\textbf{寄生虫}};

    % 添加百分比（带背景）
    \node[anchor=west,fill=white,inner sep=2pt,text=bar1] at (0.92*\textwidth,15.9) {\Large\textbf{90\%}};
    \node[anchor=west,fill=white,inner sep=2pt,text=bar2] at (0.07*\textwidth,12.9) {\Large\textbf{5\%}};
    \node[anchor=west,fill=white,inner sep=2pt,text=bar3] at (0.05*\textwidth,9.9) {\Large\textbf{3\%}};
    \node[anchor=west,fill=white,inner sep=2pt,text=bar4] at (0.03*\textwidth,6.9) {\Large\textbf{1\%}};
    \node[anchor=west,fill=white,inner sep=2pt,text=bar5] at (0.03*\textwidth,3.9) {\Large\textbf{1\%}};

%    % 添加底部刻度
%    \foreach \x in {0,10,...,90}
%    {
%        \draw[gray!50] (\x/100*\textwidth,2.4) -- (\x/100*\textwidth,3);
%        \node[below] at (\x/100*\textwidth,2.4) {\small \x\%};
%    }
\end{tikzpicture}
...
\begin{tikzpicture}[
    font=\normalsize,
    title/.style={font=\fontsize{8}{48}\selectfont\bfseries\color{white}},
    value/.style={font=\fontsize{5}{48}},
    reference/.style={font=\fontsize{5}{48}},
    ...
    \foreach \item/\enitem/\value/\range/\percentile/\detection/\status/\intro/\suggestion/\index in {
        ...
        {拟杆菌属}/{Bacteroides}/0.08974/{1.0578-47.3225}/2\%/99.04\%/偏低$\downarrow$/{革兰氏阴性厌氧菌，能降解复杂碳水化合物，产生丙酸盐，参与胆固醇代谢，调节宿主免疫系统}/{建议增加全谷物、豆类等膳食纤维摄入}/\currentpos,
    ...
    }
    {
        ...
        % 菌种名称
        \node[cell, align=left] at (0.5,\basepos) {
            \fontsize{10}{48}\selectfont\text{\textbf{\item}}\\[-0.2em]
            {\color{gray}\small\enitem}
        };

        % 正常范围
        \node[reference] at (4.5,\basepos) {\fontsize{9}{48}\selectfont\range};
        ...
        % 检测丰度值
        \node[value] at (7.5,{\basepos-\valuebarspace+0.6}) {\fontsize{8}{48}\selectfont\value};
        ...
        % 计算进度条长度和颜色
        \pgfmathsetmacro{\progress}{min(\value/\maxval, 1.0)}
        \pgfmathparse{\value > \maxval ? "customred" : (\value < \minval ? "customred" : "green!90!black")}
        \let\barcolor=\pgfmathresult
...
%        % 结果评价
%        \ifnum\pdfstrcmp{\status}{超标}=0
%            \node[value, text=customred] at (10.5,\basepos) {\fontsize{10}{48}\selectfont\textbf{\status}};
%        \else
%            \node[value, text=customGreen] at (10.5,\basepos) {\fontsize{10}{48}\selectfont\textbf{\status}};
%        \fi
...
\node[value, text=customGreen] at (10.5,\basepos) {\fontsize{10}{48}\selectfont\textbf{正常}};
\else\ifnum\pdfstrcmp{\status}{未检出}=0
    % 未检出

\node[value, text=gray] at (10.5,\basepos) {\fontsize{10}{48}\selectfont\textbf{未检出}};
\else\ifnum\pdfstrcmp{\status}{超标}=0
    % 超标

\node[value, text=customRed] at (10.5,\basepos) {\fontsize{10}{48}\selectfont\textbf{超标}};
\else\ifnum\pdfstrcmp{\status}{偏低$\downarrow$}=0
    % 偏低

\node[value, text=orange] at (10.5,\basepos) {\fontsize{10}{48}\selectfont\textbf{偏低$\downarrow$}};
\else
    % 其他情况

\node[value, text=black] at (10.5,\basepos) {\fontsize{10}{48}\selectfont\textbf{未知状态}};
\fi\fi\fi\fi



        % 丰度超过%人
        \node[value] at (13,\basepos) {\fontsize{9}{48}\selectfont\percentile};

        % %正常人检出
        \node[value] at (16,\basepos) {\fontsize{9}{48}\selectfont\detection};
        ...
        \begin{scope}[shift={(0,\cardypos)}]
            ...
            \fill[rounded corners=5pt, customTeal!15, draw=gray!5]
                (0.3,-\cardheight) rectangle (17.3,0);

            % 菌群简介图标和内容
            \node[anchor=west] at (0.5,-0.3) {
                \textbf{\color{orange}\footnotesize \textcolor{customTeal}{\faInfoCircle}}
            };
            \node[anchor=west, text width=16cm] at (0.9,-0.3) {
                {\small\color{black!80}\footnotesize \intro}
            };
            ...
            \ifnum\pdfstrcmp{\status}{超标}=0
                ...
                \node[anchor=west, text width=16cm] at (0.9,-0.8) {
                    {\small\color{black!80}\footnotesize \suggestion}
                };
            \fi
        \end{scope}
...
\begin{tikzpicture}[
    ...
    \foreach \item/\recommend/\energy/\protein/\fat/\carbs/\starch/\fiber/\cholesterol/\index in {
        ...
        {葡萄干浆即食谷物}/{0}/{1354}/{7}/{1}/{78}/{0}/{0}/{13}/10.5,
    ...
    }
    {
        ...
        % 项目名
        \node[cell, align=left] at (0.5,\ypos) {\fontsize{10}{48}\selectfont \item};
        ...
        % 其他数值显示
        \node[value] at (5.0,\ypos) {\small\energy};
...
{\fontsize{10}{48}\selectfont{\color{customTeal}\faInfoCircle}
...
% 计算表格高度
\newcommand{\calculateTableHeight}{
    \pgfmathsetmacro{\rowheight}{0.8} % 每行的高度
    \pgfmathsetmacro{\extraHeight}{0.4} % 额外的高度
    \pgfmathsetmacro{\numRows}{8} % 行数
    \pgfmathsetmacro{\tableHeight}{\rowheight*\numRows+\extraHeight} % 计算总高度
    \xdef\calculatedHeight{\tableHeight} % 将计算结果保存到变量
}

% 绘制表格
\newcommand{\drawFoodTable}{
    \begin{center}
    \begin{tikzpicture}[
        font=\normalsize,
        title/.style={font=\fontsize{8}{48}\selectfont\bfseries\color{white}},
        ...
        \def\cardwidth{\textwidth}
        \def\cardheight{\calculatedHeight} % 使用计算的高度
        \def\barheight{0.25}
        \def\barwidth{1.5}
        \def\valuebarspace{0.4}
        ...
        % 数据行和卡片
        \foreach \item/\enitem/\value/\range/\percentile/\detection/\status/\intro/\suggestion/\index in \fooddata
        {
            % 计算当前行的基础位置
            \pgfmathsetmacro{\basepos}{-1.5*\currentpos} % 调整行间距
            ...
            % 更新位置计数器
            \pgfmathsetmacro{\nextpos}{\currentpos+1} % 增加行间距
            \xdef\currentpos{\nextpos}
        }

        % 分割线
        \draw[gray!20] (0.2,-0.1) -- (\cardwidth-0.2,-0.1);
